...
async function main() {
  ...
  await waitForAction(sender, 'hello-ack');
  ...
  sender.on('message', (buf) => {
    try {
      ...
      if (data.action === 'direct-delivered') delivered += 1;
    } catch {}
  });
  ...
  receiver.on('message', (buf) => {
    try {
      const data = JSON.parse(buf.toString());
      if (data.action === 'direct-message') received += 1;
    } catch {}
  });
  ...
  const page1 = await waitForAction(sender, 'history-response', 20000);
  ...
  if (Array.isArray(page1.messages) && page1.messages.length > 0) {
    ...
    page2 = await waitForAction(sender, 'history-response', 20000);
    h2Ms = nowMs() - h2Start;
  }
...
main().catch((err) => {
  console.error('BENCH_ERR', err && err.stack ? err.stack : err);
  process.exit(1);
});
